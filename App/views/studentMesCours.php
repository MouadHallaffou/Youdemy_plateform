...
$coursesaccepted = isset($coursesaccepted) && is_array($coursesaccepted) ? $coursesaccepted : [];
...
if ($studentId) {
    $mesCourses = Student::getAcceptedCourses($pdo, $studentId);
} else {
    $mesCourses = []; 
}

// Paginate des courses
$totalCourses = count($mesCourses);
$totalPages = ceil($totalCourses / $itemsPerPage);
$startIndex = ($currentPage - 1) * $itemsPerPage;
$pagedCourses = array_slice($mesCourses, $startIndex, $itemsPerPage);
?>
...
<head>
    ...
    <style>
        /* Popup styling */
        #course-popup { z-index: 1000; }
        #course-popup .rounded-lg { max-height: 90vh; }
        .hidden { display: none; }
    </style>
</head>
...
<nav class="fixed w-full top-0 z-50 px-4 py-2 flex justify-between items-center bg-white dark:bg-gray-800 border-b-2 dark:border-gray-600">
    ...
    <div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16 mt-10">
        <h2 class="text-xl font-bold mb-5 text-white">Mes Cours Inscrits</h2>
        <div id="enrolled-courses" class="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-10">
            <?php if (!empty($pagedCourses)): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p>Aucun cours accepté trouvé.</p>
            <?php endif; ?>
        </div>
